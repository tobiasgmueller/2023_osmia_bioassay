...
df <- read_csv("input/2023_bioassay_datasheets_combined.csv", skip_empty_rows = TRUE)%>%
  ...
  mutate(days_to_death = as.numeric(days_to_death),
         days_to_event = as.numeric(days_to_event))# and finally fix difftime to num
head(df)
...
```{r graphs for atkinson application, include = FALSE}
...
df %>%
  filter(end_status=="cocoon", treatment %in% c("Osmia pollen (control)", "Osmia pollen with high antibiotics","Sterilized Osmia pollen", "Sterilized Osmia pollen with high antibiotics"))%>%
  ggplot(aes(x=days_to_event))+
  geom_density(aes(fill=treatment), alpha=.4)+
  geom_density(data=only_control, aes(fill=control), alpha =.2)+
  facet_wrap(~treatment)+
